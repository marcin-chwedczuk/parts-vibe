...
import app.partsvibe.uicomponents.breadcrumbs.BreadcrumbsModel;
...
class BreadcrumbsElementTagProcessor extends AbstractElementModelProcessor {
    private static final String TAG_NAME = "breadcrumbs";
    private static final String TEMPLATE_NAME = "ui/components/breadcrumbs";
    private static final String DATA_VARIABLE = "data";
    private final String dialectPrefix;

    BreadcrumbsElementTagProcessor(String dialectPrefix) {
    ...
    protected void doProcess(ITemplateContext context, IModel model, IElementModelStructureHandler structureHandler) {
        ...
        if (tag == null) {
            throw new TemplateProcessingException("No element tag found for <app:breadcrumbs>");
        }
        ...
        if (!(data instanceof BreadcrumbsModel breadcrumbsModel)) {
            throw new TemplateProcessingException("Attribute '" + dialectPrefix
                    + ":data' on <app:breadcrumbs> must resolve to " + BreadcrumbsModel.class.getSimpleName()
                    + ", got: " + data.getClass().getName());
        }
        structureHandler.setLocalVariable(DATA_VARIABLE, breadcrumbsModel);
    ...
    private String requireAttr(IProcessableElementTag tag, String name) {
        ...
        if (value == null || value.isBlank()) {
            throw new TemplateProcessingException(
                    "Missing required attribute '" + dialectPrefix + ":" + name + "' on <app:breadcrumbs>");
        }
        return value;
    }
